...
using namespace std;
    ...
    ImageHelper::ImageHelper(void)
    {

    }
    std::string ImageHelper::convertToStdString( QString & s )
    {
        std::string stds = s.toLocal8Bit().constData();
        return stds;
    }
    cv::Mat ImageHelper::readImage(QString & s)
    {
        cv::Mat image = cv::imread(ImageHelper::convertToStdString(s), -1);
    ...
    cv::Mat ImageHelper::RGB2BGR(cv::Mat image)
    {
        cv::Mat newImage;
        int channelsNumber = image.channels();
        switch(channelsNumber)
    ...
    bool ImageHelper::writeImage(QString fileName, cv::Mat image)
    {
        bool success = false;
        cv::Mat newImage = ImageHelper::RGB2BGR(image);
        success = cv::imwrite(ImageHelper::convertToStdString(fileName), newImage);

        return success;
    }
    QImage* ImageHelper::convertToQImage( cv::Mat& matrix)
    {
        QImage* img;
        int channelsNumber;
        //Mat resized;
...
//            cvtColor(matrix, matrix, CV_BGRA2RGBA);
    ...
    vector<QImage*> ImageHelper::splitToQImage(cv::Mat matrix)
    {
    ...
    cv::Mat ImageHelper::getChannel(Channels ch, cv::Mat & matrix)
    {
        cv::Mat result;
    ...
    QRresult ImageHelper::mgs_qr(cv::Mat& A)
    {
        QRresult result;
        result.Q = A;
    ...
    template<class T>
    void ImageHelper::matIteration(cv::Mat M,  void (*callback)(T Value))
    {
    ...
    cv::Mat ImageHelper::FFT(cv::Mat image)
    {
        if (image.channels() > 1)
        {
            vector<cv::Mat> ch;
    ...
    cv::Mat ImageHelper::IFT(cv::Mat transform, int origType)
    {
        cv::Mat iftm;
    ...
    *p*(cv::log() p))/log(2));
    ...
    return !exist;*/
    //	}
cv::Scalar ImageHelper::getMSSIM( const cv::Mat& i1, const cv::Mat& i2, cv::Mat& ssim_map)
    {
...
QIcon ImageHelper::convertToQIcon(cv::Mat image, int width, int height)
{
...
double ImageHelper::calcSSIM(cv::Mat& src1, cv::Mat& src2, int channel, int method, const double K1, const double K2,	const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma)
{
    cv::Mat mask; cv::Mat map;
    return 0;
}

double ImageHelper::calcSSIM(cv::Mat& src1, cv::Mat& src2, cv::Mat& mask, cv::Mat& ssim_map, int channel, int method, const double K1, const double K2,	const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma)
{
    return 0;
...
//double ImageHelper::calcSSIMBB(cv::Mat& src1, cv::Mat& src2, int channel, int method, int boundx, int boundy, const double K1, const double K2, const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma)
//{
//    Mat ssim_map;
//    return calcSSIMBB(src1, src2, ssim_map, channel, method, boundx, boundy, K1, K2, L, downsamplewidth, gaussian_window, gaussian_sigma);
//}
//double ImageHelper::calcSSIMBB(cv::Mat& src1, cv::Mat& src2, cv::Mat& ssim_map, int channel, int method, int boundx, int boundy, const double K1, const double K2, const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma)
//{
//    IplImage Src1 = IplImage(src1);
//    IplImage Src2 = IplImage(src2);
//    IplImage Ssim_map = IplImage(ssim_map);
...
double ImageHelper::meanDiff(cv::Mat img1, cv::Mat img2)
{
    cv::Mat diff;
...
double ImageHelper::normCorrelation(cv::Mat img1, cv::Mat img2)
{
...
double ImageHelper::correlationQuality(cv::Mat img1, cv::Mat img2)
{
    cv::Mat mult;
    cv::multiply(img1, img2, mult);
...
double ImageHelper::maxDiff(cv::Mat img1, cv::Mat img2)
{
    cv::Mat diff;
    cv::absdiff(img1, img2, diff);
    double max;
...
double ImageHelper::imageFidelity(cv::Mat img1, cv::Mat img2)
{
...
template<class T>
cv::Mat ImageHelper::getO(cv::Mat img)
{
...
double ImageHelper::laplMeanSqError(cv::Mat img1, cv::Mat img2)
{
    cv::Mat oImg1,oImg2;
    switch(img1.depth())
    {
    case CV_8U:
        oImg1 = ImageHelper::getO<unsigned char>(img1);
        break;
    case CV_16U:
        oImg1 = ImageHelper::getO<unsigned int>(img1);
        break;
    case CV_32F:
        oImg1 = ImageHelper::getO<float>(img1);
    case CV_64F:
        oImg1 = ImageHelper::getO<double>(img1);
        break;
    }
    switch(img2.depth())
    {
    case CV_8U:
        oImg2 = ImageHelper::getO<unsigned char>(img2);
        break;
    case CV_16U:
        oImg2 = ImageHelper::getO<unsigned int>(img2);
        break;
    case CV_32F:
        oImg2 = ImageHelper::getO<float>(img2);
    case CV_64F:
        oImg2 = ImageHelper::getO<double>(img2);
        break;
    }
...
double ImageHelper::meanSqError(cv::Mat img1, cv::Mat img2)
{
    cv::Mat pow2;
...
double ImageHelper::peakMeanSqError(cv::Mat img1, cv::Mat img2)
{
    cv::Mat pow2;
    cv::pow(img1-img2, 2.0, pow2);
...
double ImageHelper::normalizedAbsoluteError(cv::Mat img1, cv::Mat img2)
{
    cv::Scalar result = cv::sum(img1-img2)/cv::sum(img1);
    return result.val[0];
}
double ImageHelper::normalizedMeanSquareError(cv::Mat img1, cv::Mat img2)
{
    cv::Mat img1pow2, diff;
    cv::pow(img1, 2.0, img1pow2);
    cv::absdiff(img1, img2, diff);
...
double ImageHelper::snr(cv::Mat img1, cv::Mat img2)
{
    cv::Mat img1pow2;
    cv::pow(img1, 2.0, img1pow2);
    cv::Mat img1img2pow2;
...
double ImageHelper::psnr(cv::Mat img1, cv::Mat img2)
{
