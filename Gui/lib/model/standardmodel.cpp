...
StandardModel::StandardModel(QStringList headers, QObject *parent){
...
bool StandardModel::setItemData(const QModelIndex &index, const QMap<int, QMap<int, QVariant> > &data)
{
    if(index.isValid())
    {
        AbstractItem* item = getItem(index);
...
bool StandardModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
    ...
    // first check if we drop files and can we add them
    if(data->hasUrls() && this->canAcceptFiles())
    {
        QList<QUrl> urls = data->urls();
        return this->readFiles(urls);
    } else if(data->hasFormat(getItemClassName())) {
...
bool StandardModel::canAcceptFiles() const{
    return this->_canAcceptFiles;
}
void StandardModel::setCanAcceptFiles(bool value){
    this->_canAcceptFiles = value;
}
bool StandardModel::readFiles(QList<QUrl> files){
    // overwrite;
    return false;
}
