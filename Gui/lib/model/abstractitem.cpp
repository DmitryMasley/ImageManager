...
AbstractItem::AbstractItem(QMap<int, QMap<int, QVariant> > data, QObject *parent, bool isRoot) : QObject(parent)
{
    _data = data;
    setRoot(isRoot);
}
...
bool AbstractItem::setData(int column, const QVariant &value, int role)
{
    ...
    QMap<int, QVariant> dataItem;
    if(_data.contains(column)){
        dataItem = _data.value(column);
    }
    dataItem.insert(role, QVariant(value));
    _data.insert(column, dataItem);
    return true;

}
bool AbstractItem::setItemData(QMap<int, QMap<int, QVariant> > data)
{
    _data = data;
    return true;
}
...
{
    int size = _data.size();
    _data[size+1] = dataItem;
}
QMap<int, QMap<int, QVariant> > AbstractItem::getData()
{
    return _data;
}
...
bool AbstractItem::canAcceptDrops()
{
    return (root() || (this->canHaveChildren() && this->canAcceptMoreChildren()));
}
